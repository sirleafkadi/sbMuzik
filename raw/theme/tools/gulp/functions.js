...
module.exports = {
     ...
     * @returns {*}
     */
    outputPath: function(path, outputFile) {
        ...
        if (matched) {
            ...
            outputs.forEach(function (output) {
                const outputPath = path.replace(matched[0], output).replace(outputFile, '');
                (function (_output) {
                    ...
                    for (let type in vendorObj) {
                        ...
                        switch (type) {
                            case 'fonts':
                                ...
                                gulp.src(vendorObj[type]).pipe(_this.outputPath(bundle.bundle[type] + '/' + vendor)())
                                    // .pipe(gulp.dest(finalSrc));
                                break;
                            case 'images':
                                finalSrc = src + assets + '/images/' + vendor;
                                gulp.src(vendorObj[type]).pipe(_this.outputPath(bundle.bundle[type] + '/' + vendor)())
                                    // .pipe(gulp.dest(finalSrc));
                                break;
                        }
                ...
                switch (type) {
                    case 'styles':
                        ...
                        gulp.src(bundle.src[type]).pipe(_this.cssRewriter(bundle.bundle[type])())
                            ...
                            .pipe(_this.outputPath(bundle.bundle[type], outputFile)()).pipe(sync.stream());
                        break;

                    case 'scripts':
                        finalSrc = src + assets + '/js';
                        gulp.src(bundle.src[type]).pipe(concat(outputFile)).pipe(_this.jsBuild()())
                            .pipe(_this.outputPath(bundle.bundle[type], outputFile)());
                        break;

                    case 'images':
                        gulp.src(bundle.src[type]).pipe(gulp.dest(_this.dotPath(bundle.bundle[type])));
                        break;
    ...
    copysrc: function (bundle) {
        ...
        if (typeof bundle.src !== 'undefined' && typeof bundle.output !== 'undefined') {
            for (var type in bundle.src) {
                ...
                switch (type) {
                    case 'scripts':
                        gulp.src(bundle.src[type]).pipe(_this.jsBuild()()).pipe(_this.outputPath(bundle.output[type])());
                        break;

                    case 'images':
                        finalDist = dist + assets + '/images';
                        gulp.src(bundle.src[type]).pipe(gulp.dest(finalDist));
                        break;

                    case 'audio':
                        finalDist = dist + assets + '/audio';
                        gulp.src(bundle.src[type]).pipe(gulp.dest(finalDist));
                        break;

                    default:
                        gulp.src(bundle.src[type])
                            .pipe(gulpif(config.htmlMinify, htmlmin({collapseWhitespace: true, removeComments: true})))
                            .pipe(gulp.dest(dist));
                        break;
                }
            }
